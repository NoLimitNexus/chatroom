        ...
        if ((isLocked || inventoryOpen || isMobile) && myCharacter) {
            ...
            if (boatState.active && boatState.boatGroup) {
                ...
                if (isMoving) {
                    ...
                    var cosR = Math.cos(boat.rotation.y), sinR = Math.sin(boat.rotation.y);

                    // Get average terrain height at current and next position
                    var currDepthSum = 0, nextDepthSum = 0;
                    var nextOnLand = false;
                    for (var hi = 0; hi < hullOffsets.length; hi++) {
                        var ox = hullOffsets[hi][0], oz = hullOffsets[hi][1];
                        var rx = ox * cosR - oz * sinR;
                        var rz = ox * sinR + oz * cosR;
                        currDepthSum += getTerrainHeight(boat.position.x + rx, boat.position.z + rz);
                        var nh = getTerrainHeight(nextX + rx, nextZ + rz);
                        nextDepthSum += nh;
                        if (nh > -1.3) nextOnLand = true;
                    }
                    var currAvg = currDepthSum / hullOffsets.length;
                    var nextAvg = nextDepthSum / hullOffsets.length;

                    // Allow movement if: heading to deeper water OR not touching land
                    var headingDeeper = nextAvg < currAvg - 0.01;
                    var canMove = !nextOnLand || headingDeeper;
